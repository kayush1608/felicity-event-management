...
exports.acceptMember = async (req, res) => {
  try {
    ...
    const acceptedCount = 1 + team.members.filter((m) => m.status === 'Accepted').length;
    const required = Number(team.teamSize?.min || 0);
    if (acceptedCount >= required) {
      team.isComplete = true;
      team.status = 'Complete';
      await team.save();

      const userIds = [team.teamLeaderId.toString(), ...team.members.filter((m) => m.status === 'Accepted').map((m) => m.userId.toString())];
      ...
      for (const uid of userIds) {
        ...
        try {
          ...
          if (user?.email) {
            await sendTicketEmail(user.email, { ticketId, event, registration: reg });
          }
        } catch (e) {
...
exports.getTeam = async (req, res) => {
  try {
    const { id } = req.params;
    const team = await Team.findById(id).populate('members.userId', 'name email').populate('teamLeaderId', 'name email');
    if (!team) return res.status(404).json({ success: false, message: 'Team not found' });
    return res.json({ success: true, data: team });
  } catch (error) {
